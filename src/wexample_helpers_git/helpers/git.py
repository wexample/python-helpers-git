...
def git_current_branch(*, cwd: FileStringOrPath, inherit_stdio: bool = False) -> str:
    ...
    return (
        shell_run(["git", "rev-parse", "--abbrev-ref", "HEAD"], inherit_stdio=inherit_stdio, cwd=file_resolve_path(cwd))
        .stdout.strip()
    )
...
def git_set_upstream(branch: str, *, cwd: FileStringOrPath, remote: str = "origin", inherit_stdio: bool = True) -> None:
...
def git_pull_rebase_autostash(*, cwd: FileStringOrPath, inherit_stdio: bool = True) -> None:
    """Pull latest changes with rebase and autostash to preserve local modifications."""
    shell_run(["git", "pull", "--rebase", "--autostash"], inherit_stdio=inherit_stdio, cwd=file_resolve_path(cwd))


def git_has_working_changes(*, cwd: FileStringOrPath) -> bool:
    """Return True if there are unstaged changes in tracked files."""
    out = shell_run(["bash", "-lc", "git diff --quiet || echo CHANGED"], inherit_stdio=False, cwd=file_resolve_path(cwd)).stdout.strip()
    return out == "CHANGED"


def git_has_index_changes(*, cwd: FileStringOrPath) -> bool:
    """Return True if there are staged (indexed) changes."""
    out = shell_run(["bash", "-lc", "git diff --cached --quiet || echo CHANGED"], inherit_stdio=False, cwd=file_resolve_path(cwd)).stdout.strip()
    return out == "CHANGED"


def git_commit_all_with_message(message: str, *, cwd: FileStringOrPath, inherit_stdio: bool = True) -> None:
    """Commit all tracked changes with the provided message if any are present (callers should check)."""
    shell_run(["git", "commit", "-am", message], inherit_stdio=inherit_stdio, cwd=file_resolve_path(cwd))
...
def git_push_follow_tags(*, cwd: FileStringOrPath, inherit_stdio: bool = True) -> None:
    ...
    shell_run(["git", "push", "--follow-tags"], inherit_stdio=inherit_stdio, cwd=file_resolve_path(cwd))


def git_ensure_upstream(*, cwd: FileStringOrPath, default_remote: str = "origin", inherit_stdio: bool = True) -> str:
    ...
    if not upstream:
        git_set_upstream(branch, cwd=cwd_resolved, remote=default_remote, inherit_stdio=inherit_stdio)
